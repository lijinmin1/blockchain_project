...
contract Supplychain{
    ...
    function SetBank(string memory _name, address _ad, uint _amount) public returns(bool) {
    ...
    function SetCore(string memory _name, address _ad, uint _amount) public returns(bool) {
        // require(_ad == msg.sender);
    ...
    function AddSME(string memory _name, address _ad, uint _amount) public returns(bool) {
        // require(_ad == msg.sender);
    ...
    //中小企业发起应收账款单据
    function IssueReceipt(address SME, address core, uint amount, uint timeInterval, string memory info) public returns(uint id_) {
    ...
    function PayForReceipt(address core, address SME, uint receiptID, uint amount) public returns(bool) {
        Receipt memory r;
        uint i;
        ...
        for (i = 0; i < len; i++) {
            ...
            require(i != receipts[SME].length - 1,"no such receipt id.");
        }
